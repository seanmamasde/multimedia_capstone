    ...
    const fetchCourtUsage = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // 將 teams 的 tid 放入對應場地
          data.forEach((record) => {
            console.log(record.teams);
            if (record.teams) {
              for (const [teamId, court] of Object.entries(record.teams)) {
                if (courts[court]) {
                  courts[court].push(teamId);
                }
              }
            }
          });
          ...
          setError(data.error || '無法擷取場地使用情形');
        }
      } catch (err) {
        setError('伺服器錯誤，無法擷取資料');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="home-container">
      ...
      <div className="content-wrapper">
        ...
        <h2 className="title">當天場地使用情形</h2>
        {loading ? (
          <p>載入中...</p>
        ) : error ? (
